		...
		foreach ( $assigntypes as $atype ) {
			...
			$this->settings[$setting_name . '_thumb'] = 
				array(
					...
					'desc'    => __( '<hr /><p>&nbsp;</p>' ),
					'std'     =>  'https://placehold.it/' . bank106_option('thumb_w') . 'x' . bank106_option('thumb_h'),
					'type'    => 'medialoader',
					'section' => 'types'
				);
		...
		switch ( $type ) {
			...
			case 'medialoader':
				...
				if ( strpos ( $options[$id], 'http') !==false ) {
					echo '<img id="previewimage_' . $id . '" src="' . $options[$id] . '" width="' . (bank106_option('thumb_w') ) . '" height="' . (bank106_option('thumb_h') ) . '" alt="default thumbnail" />';
				} else {
					echo '<img id="previewimage_' . $id . '" src="https://placehold.it/' . (bank106_option('thumb_w') ) . 'x' . (bank106_option('thumb_h') ) . '" alt="default thumbnail" />';
				}
